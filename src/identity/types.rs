...
pub struct SpiffeId {
    ...
    /// Represents a complete service identity, including certificate and SPIFFE identity
...
impl SpiffeId {
    ...
    pub fn from_uri(uri: &str) -> Result<Self, Error> {
        // For example: "spiffe://tenant-a/service-b"
    ...
    /// Create SPIFFE URI
    pub fn create_uri(tenant: &str, service: &str) -> String {
        format!("spiffe://{}/{}", tenant, service)
    }
    
    /// Create new SPIFFE ID
    pub fn new(tenant: &str, service: &str) -> Self {
        let uri = Self::create_uri(tenant, service);
        Self {
            uri,
...
impl ServiceIdentity {
    ...
    pub fn status(&self) -> IdentityStatus {
        ...
        if now < self.issued_at {
            ...
            // Certificate valid (not considering revocation status, additional check needed)
