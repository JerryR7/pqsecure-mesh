...
pub trait PolicyStore: Send + Sync {
    /// Get policy
    async fn get_policy(&self, tenant: &str) -> Result<AccessPolicy, Error>;
    
    /// Update policy
    async fn update_policy(&self, policy: AccessPolicy) -> Result<(), Error>;
...
impl FilePolicyStore {
    ...
    pub fn new(config: Arc<Config>) -> Self {
        ...
        // Here we use a blocking method to load the initial policies instead of async
        // In real applications, consider using non-blocking methods or block_on during startup
        let runtime = tokio::runtime::Runtime::new().unwrap();
        let _ = runtime.block_on(store.reload());
    ...
    async fn load_policy_from_file(path: &PathBuf) -> Result<AccessPolicy, Error> {
    ...
    /// Save policy to file
    async fn save_policy_to_file(&self, policy: &AccessPolicy, tenant: &str) -> Result<(), Error> {
