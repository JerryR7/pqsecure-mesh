...
use crate::identity::{IdentityProvider, IdentityStatus, ServiceIdentity};
...
impl RotationController {
    ...
    pub fn new(config: Arc<Config>, identity_provider: Arc<dyn IdentityProvider>) -> Self {
        Self {
            config,
            identity_provider,
    ...
    async fn check_identity(&self, spiffe_id: &str) -> Result<(), Error> {
        ...
        match self
            .identity_provider
            .check_identity_status(&identity)
            .await?
        {
            ...
            IdentityStatus::Revoked => {
                warn!(
                    "Identity revoked, removing from managed identities: {}",
                    spiffe_id
                );
            ...
            IdentityStatus::Unknown => {
                warn!(
                    "Identity status unknown, removing from managed identities: {}",
                    spiffe_id
                );
    ...
    async fn rotate_identity(&self, identity: &ServiceIdentity) -> Result<(), Error> {
        ...
        let mut identities = self.managed_identities.write().unwrap();
        identities.insert(
            new_identity.spiffe_id.uri.clone(),
            ManagedIdentity {
